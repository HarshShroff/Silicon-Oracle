...
@login_required
def portfolio():
    return redirect(url_for("sentinel_ui.simulation"))
        ...
        if "save_api_keys" in request.form:
            ...
            try:
                ...
                if quote and quote.get("source") == "finnhub":
                    results["Finnhub"] = {"success": True, "message": "Connected successfully"}
                else:
                    results["Finnhub"] = {"success": False, "error": "Invalid API key"}
            except Exception as e:
                results["Finnhub"] = {"success": False, "error": str(e)}
            results["Gemini"] = {"success": True, "message": "Tested during AI analysis"}
...
def macro():
    ...
    return render_template("pages/macro.html")
