...
class StockService:
    ...
    def get_realtime_quote(self, ticker: str) -> Optional[Dict[str, float]]:
        ...
        if not self._finnhub_client:
            logger.warning(
                f"StockService: No Finnhub client for {ticker}. Access denied.")
            return None
    ...
    def get_technical_indicators(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        if len(df) < 50:
            logger.warning(
                f"Insufficient data for {ticker}: only {len(df)} rows")
            return None
        ...
        try:
            ...
            perf_1m = float(
                (current_price / df['Close'].iloc[-21] - 1) * 100) if len(df) > 21 else 0
            perf_3m = float(
                (current_price / df['Close'].iloc[-63] - 1) * 100) if len(df) > 63 else 0
    ...
    def get_market_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'session': 'regular' if (is_weekday and is_market_hours) else 'closed',
            'holiday': None
        }
    ...
    def get_complete_data(self, ticker: str) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_key = {executor.submit(
                func): key for key, func in tasks.items()}
