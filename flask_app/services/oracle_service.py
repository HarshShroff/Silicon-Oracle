...
class OracleService:
    ...
    def __init__(self, config: Dict[str, str] = None):
        ...
        self.spy_data = None  # Cache SPY data for market context
    ...
    def calculate_oracle_score(self, ticker: str) -> Dict[str, Any]:
        ...
        def add_factor(name: str, signal: str, detail: str,
                       points: float, max_points: float, has_data: bool = True):
        ...
        if sma_50 and current_price:
            if current_price > sma_50:
                add_factor('Trend', 'Bullish',
                           'Price above SMA50 (Uptrend)', 1.0, 1.0)
            else:
                add_factor('Trend', 'Bearish',
                           'Price below SMA50 (Downtrend)', 0.0, 1.0)
        else:
            add_factor('Trend', 'N/A', 'Insufficient technical data',
                       0, 0, has_data=False)
        ...
        if rsi is not None:
            if 30 <= rsi <= 70:
                add_factor('Momentum', 'Bullish',
                           f'RSI {rsi:.0f} - Healthy range', 1.0, 1.0)
            elif rsi < 30:
                add_factor('Momentum', 'Neutral',
                           f'RSI {rsi:.0f} - Oversold', 0.5, 1.0)
            else:
                add_factor('Momentum', 'Bearish',
                           f'RSI {rsi:.0f} - Overbought', 0.0, 1.0)
        else:
        ...
        if spy_price and spy_sma:
            if spy_price > spy_sma:
                add_factor('Market', 'Bullish',
                           'S&P 500 in uptrend (Risk-On)', 1.0, 1.0)
            else:
                add_factor('Market', 'Bearish',
                           'S&P 500 weak (Risk-Off)', 0.0, 1.0)
        else:
            ...
            if total > 0:
                ...
                if bullish_pct >= 60:
                    add_factor('Analysts', 'Bullish',
                               f'{bullish_pct:.0f}% Bullish', 1.0, 1.0)
                elif bullish_pct >= 40:
                    add_factor('Analysts', 'Neutral',
                               f'{bullish_pct:.0f}% Bullish', 0.5, 1.0)
                else:
                    add_factor('Analysts', 'Bearish',
                               f'Only {bullish_pct:.0f}% Bullish', 0.0, 1.0)
            else:
                add_factor('Analysts', 'N/A', 'No analyst coverage',
                           0, 0, has_data=False)
        else:
        ...
        if insiders and len(insiders) > 0:
            buy_value = sum([t.get('share', 0) * t.get('transactionPrice', 0)
                             for t in insiders if t.get('change', 0) > 0])
            sell_value = sum([abs(t.get('share', 0)) * t.get('transactionPrice', 0)
                              for t in insiders if t.get('change', 0) < 0])

            if buy_value > sell_value:
                add_factor('Insiders', 'Bullish',
                           'Net insider buying detected', 1.0, 1.0)
            elif sell_value > buy_value * 2:
                add_factor('Insiders', 'Bearish',
                           'Heavy insider selling', 0.0, 1.0)
            else:
                add_factor('Insiders', 'Neutral',
                           'Mixed insider activity', 0.5, 1.0)
        else:
            add_factor('Insiders', 'N/A',
                       'No insider data (ETF/Fund)', 0, 0, has_data=False)

        # ============================================
        # FACTOR 6: PRICE TARGET UPSIDE (Max: 1.0)
        # ============================================
        target_mean = price_targets.get(
            'target_mean') or company.get('target_price')
        ...
        if current_price and target_mean and current_price > 0 and target_mean > 0:
            ...
            if upside_pct >= 15:
                add_factor('Target', 'Bullish',
                           f'{upside_pct:+.1f}% upside to target', 1.0, 1.0)
            elif upside_pct >= 5:
                add_factor('Target', 'Neutral',
                           f'{upside_pct:+.1f}% modest upside', 0.5, 1.0)
            elif upside_pct >= -5:
                add_factor('Target', 'Neutral',
                           f'{upside_pct:+.1f}% near target', 0.5, 1.0)
            else:
                add_factor('Target', 'Bearish',
                           f'{upside_pct:.1f}% downside risk', 0.0, 1.0)
        else:
        ...
        if peg and peg > 0:
            if peg < 1:
                add_factor('Valuation', 'Bullish',
                           f'PEG {peg:.2f} - Undervalued', 1.0, 1.0)
            elif peg < 2:
                add_factor('Valuation', 'Neutral',
                           f'PEG {peg:.2f} - Fair value', 0.5, 1.0)
            else:
                add_factor('Valuation', 'Bearish',
                           f'PEG {peg:.2f} - Expensive', 0.0, 1.0)
        elif pe and pe > 0:
            if pe < 15:
                add_factor('Valuation', 'Bullish',
                           f'P/E {pe:.1f} - Low', 0.75, 1.0)
            elif pe < 25:
                add_factor('Valuation', 'Neutral',
                           f'P/E {pe:.1f} - Fair', 0.5, 1.0)
            else:
                add_factor('Valuation', 'Neutral',
                           f'P/E {pe:.1f} - Growth stock', 0.25, 1.0)
        else:
            add_factor('Valuation', 'N/A', 'No valuation data',
                       0, 0, has_data=False)
            ...
            try:
                ...
                if 7 <= days_until <= 30:
                    add_factor('Earnings', 'Caution',
                               f'Earnings in {days_until}d (Catalyst)', 0.5, 0.5)
                elif 0 <= days_until < 7:
                    add_factor(
                        'Earnings', 'Caution', f'Earnings in {days_until}d (High Volatility)', 0.0, 0.5)
                elif days_until > 30:
                    add_factor('Earnings', 'Neutral',
                               f'Earnings in {days_until}d', 0.4, 0.5)
                else:
                    add_factor('Earnings', 'Clear',
                               'Earnings passed', 0.25, 0.5)
            except Exception:
                add_factor('Earnings', 'N/A', 'Date parse error',
                           0, 0, has_data=False)
        else:
            add_factor('Earnings', 'N/A', 'No earnings data (ETF)',
                       0, 0, has_data=False)
        ...
        if news and len(news) >= 3:
            bullish_words = ['surge', 'gain', 'beat', 'upgrade',
                             'bullish', 'buy', 'strong', 'growth', 'rally']
            bearish_words = ['drop', 'fall', 'miss', 'downgrade',
                             'bearish', 'sell', 'weak', 'loss', 'decline']
            ...
            if pos_count > neg_count:
                add_factor('News', 'Bullish',
                           'Positive media coverage', 1.0, 1.0)
            elif neg_count > pos_count:
                add_factor('News', 'Bearish',
                           'Negative media coverage', 0.0, 1.0)
            else:
                add_factor('News', 'Neutral',
                           'Mixed media sentiment', 0.5, 1.0)
        else:
        ...
        if div_yield and div_yield > 0:
            ...
            if div_pct >= 3:
                add_factor('Dividend', 'Bullish',
                           f'{div_pct:.2f}% yield (Income)', 0.5, 0.5)
            elif div_pct >= 1:
                add_factor('Dividend', 'Neutral',
                           f'{div_pct:.2f}% yield', 0.25, 0.5)
            else:
                add_factor('Dividend', 'Neutral',
                           f'{div_pct:.2f}% yield (Low)', 0.1, 0.5)
        else:
            add_factor('Dividend', 'N/A', 'No dividend (Growth)',
                       0, 0, has_data=False)
        ...
        if volatility is not None:
            if volatility < 20:
                add_factor('Volatility', 'Bullish',
                           f'{volatility:.1f}% - Low risk', 0.5, 0.5)
            elif volatility < 40:
                add_factor('Volatility', 'Neutral',
                           f'{volatility:.1f}% - Moderate', 0.25, 0.5)
            else:
                add_factor('Volatility', 'Bearish',
                           f'{volatility:.1f}% - High risk', 0.0, 0.5)
        else:
            add_factor('Volatility', 'N/A', 'No volatility data',
                       0, 0, has_data=False)
        ...
        if current_price and year_high > year_low > 0:
            ...
            if position < 0.3:
                add_factor('52W Range', 'Bullish',
                           'Near 52-week low', 0.5, 0.5)
            elif position > 0.8:
                add_factor('52W Range', 'Caution',
                           'Near 52-week high', 0.0, 0.5)
            else:
                add_factor('52W Range', 'Neutral', 'Mid-range', 0.25, 0.5)
        else:
            add_factor('52W Range', 'N/A', 'No range data',
                       0, 0, has_data=False)
